...
def get_detalles_evaluacion(request, pk):
    ...
    try:
        ...
        # Añadir advertencia si no hay ejercicios
        if len(ejercicios) == 0:
            print(f"Advertencia: La evaluación ID {pk} no tiene ejercicios asignados")
            
        # Obtener información del creador (docente)
        creador_info = {}
        try:
            profile = evaluacion.creador.profile
            creador_info = {
                'id': evaluacion.creador.id,
                'username': evaluacion.creador.username,
    ...
    try:
        ...
        for resultado in resultados:
            ...
            try:
                ...
                codigo_ejercicio = next(
                    (ej.get('codigo', '') 
                     for ej in ejercicios if ej.get('ejercicio_id') == ejercicio_id), 
                    ''
                )
...
def obtener_historial_evaluaciones(request):
    ...
    try:
        ...
        for historial in historiales:
            ...
            respuestas_data = []
            for respuesta in historial.detalles.get('respuestas', []):
                respuestas_data.append({
                    'ejercicio_id': respuesta['ejercicio_id'],
                    'ejercicio_titulo': respuesta['ejercicio_titulo'],
                    'puntaje_obtenido': respuesta['puntaje_obtenido'],
                    'puntaje_maximo': respuesta['ejercicio_puntaje'],
            ...
            eval_data = {
                ...
                'puntaje': historial.puntaje_total,
...
def obtener_evaluacion_historial(request, historial_id):
    """
    Obtiene una evaluación específica desde el historial con datos completos de ejercicios
    """
    try:
        estudiante_id = request.query_params.get('estudiante_id')
        ...
        if is_docente:
            # Los docentes pueden ver cualquier historial sin filtrar por estudiante_id
            historial = HistorialEvaluacion.objects.filter(id=historial_id).first()
        else:
            # Los estudiantes solo pueden ver su propio historial
            if not estudiante_id:
                estudiante_id = request.user.id
        ...
        if not historial:
            raise Http404("No HistorialEvaluacion matches the given query.")
        ...
        evaluacion_data = {
            ...
            'duracion_minutos': historial.detalles.get('duracion_minutos', 0),
            'fecha_inicio': historial.fecha_inicio.isoformat() if historial.fecha_inicio else None,
            'fecha_fin': historial.fecha_fin.isoformat() if historial.fecha_fin else None,
            'codigo_acceso': historial.evaluacion_codigo_acceso,
            'estado': 'finalizada',  # Siempre finalizada para historial
            'puntaje_total': historial.puntaje_total,
            'puntaje_aprobacion': historial.detalles.get('puntaje_aprobacion', 60),
            'creador': {
                'id': historial.docente_id,
                'username': historial.docente_nombre,
                'rol': 'docente'
            },
            'ejercicios': []  # Se llenará a continuación
        }
        
        # Procesar ejercicios desde el historial
        if 'respuestas' in historial.detalles:
            ejercicios = []
            for i, respuesta in enumerate(historial.detalles['respuestas']):
                # Validar que tengamos todos los campos necesarios
                if not respuesta or 'ejercicio_id' not in respuesta:
                    continue
                    
                # Crear objeto de ejercicio con todos los campos necesarios
                ejercicio = {
                    'id': respuesta.get('ejercicio_id'),
                    'titulo': respuesta.get('ejercicio_titulo', f'Ejercicio {i+1}'),
                    'descripcion': respuesta.get('ejercicio_descripcion', ''),
                    'puntaje': respuesta.get('ejercicio_puntaje', 10),
                    'codigo': respuesta.get('codigo', ''),
                    'es_correcta': respuesta.get('es_correcta', False),
                    'puntaje_obtenido': respuesta.get('puntaje_obtenido', 0),
                    'puntaje_maximo': respuesta.get('ejercicio_puntaje', 10),
                ...
                # Intentar extraer información más detallada si existe
                resultados = respuesta.get('resultados', [])
                if resultados and isinstance(resultados, list):
                    # Ejemplos básicos basados en los resultados
                    ejemplos = []
                    for idx, result in enumerate(resultados[:3]):  # Tomar solo los primeros 3 para ejemplos
                        if isinstance(result, dict) and 'input' in result and 'expected' in result:
                            ejemplos.append({
                                'entrada': result.get('input', ''),
                                'salida': result.get('expected', '')
                            })
                    
                    if ejemplos:
                        ejercicio['ejemplos'] = ejemplos
                        ejercicio['contenido']['ejemplos'] = ejemplos
                
                ejercicios.append(ejercicio)
            
            evaluacion_data['ejercicios'] = ejercicios
        
        # Si no hay respuestas, intentar recuperar ejercicios originales de la evaluación
        if not evaluacion_data['ejercicios'] and historial.evaluacion_id:
            try:
                ...
                # Convertir ejercicios originales al formato esperado
            ...
            except (Evaluacion.DoesNotExist, Exception) as e:
                # Silenciar errores, solo estamos intentando recuperar datos adicionales
                logger.warning(f"No se pudieron obtener ejercicios originales: {str(e)}")
        
        # Log para debugging
        logger.info(f"Retornando {len(evaluacion_data['ejercicios'])} ejercicios en historial ID {historial_id}")
...
def guardar_evaluacion_en_historial(estudiante_evaluacion):
    ...
    try:
        ...
        try:
            docente_profile = evaluacion.creador.profile
            docente_nombre = f"{docente_profile.nombres} {docente_profile.apellidos}"
        except:
            docente_nombre = evaluacion.creador.username
        
        # Obtener perfil del estudiante
        try:
            estudiante_profile = estudiante.profile
            estudiante_nombre = f"{estudiante_profile.nombres} {estudiante_profile.apellidos}"
        except:
            estudiante_nombre = estudiante.username
        ...
        for respuesta in respuestas:
            ...
            # Extraer información del contenido del ejercicio
            contenido_ejercicio = {}
            if hasattr(ejercicio, 'contenido') and ejercicio.contenido:
                if isinstance(ejercicio.contenido, str):
                    try:
                        contenido_ejercicio = json.loads(ejercicio.contenido)
                    except:
                        contenido_ejercicio = {}
                elif isinstance(ejercicio.contenido, dict):
                    contenido_ejercicio = ejercicio.contenido
            
            # Extraer ejemplos si existen
            ejemplos = []
            if contenido_ejercicio and 'ejemplos' in contenido_ejercicio:
            ...
            respuesta_data = {
                ...
                'ejercicio_puntaje': ejercicio.puntaje,
                'puntaje_obtenido': respuesta.puntaje_obtenido,
                'es_correcta': respuesta.es_correcta,
                ...
                'formatos_entrada': contenido_ejercicio.get('formatos_entrada', []),
                'pista': contenido_ejercicio.get('pista', ''),
                'etiquetas': contenido_ejercicio.get('etiquetas', []),
                'template': contenido_ejercicio.get('template', ''),
            }
            
            # Extraer código y resultados del objeto respuesta
            if hasattr(respuesta, 'respuesta') and respuesta.respuesta:
                if isinstance(respuesta.respuesta, str):
                    try:
                        respuesta_json = json.loads(respuesta.respuesta)
                        if isinstance(respuesta_json, dict):
                            # Añadir código
                            if 'codigo' in respuesta_json:
                                respuesta_data['codigo'] = respuesta_json['codigo']
                            
                            # Añadir resultados
                            if 'resultados' in respuesta_json:
                                respuesta_data['resultados'] = respuesta_json['resultados']
                            
                            # Añadir stderr si existe
                            if 'stderr' in respuesta_json:
                                respuesta_data['stderr'] = respuesta_json['stderr']
                    except json.JSONDecodeError:
                        # Si no es JSON válido, guardar como está
                        respuesta_data['respuesta_raw'] = respuesta.respuesta
                elif isinstance(respuesta.respuesta, dict):
                    # Añadir código
                    if 'codigo' in respuesta.respuesta:
                        respuesta_data['codigo'] = respuesta.respuesta['codigo']
                    
                    # Añadir resultados
                    if 'resultados' in respuesta.respuesta:
                        respuesta_data['resultados'] = respuesta.respuesta['resultados']
                    
                    # Añadir stderr si existe
                    if 'stderr' in respuesta.respuesta:
                        respuesta_data['stderr'] = respuesta.respuesta['stderr']
            
            # Asegurar que hay código presente
            if 'codigo' not in respuesta_data:
                respuesta_data['codigo'] = contenido_ejercicio.get('template', '')
            
            respuestas_data.append(respuesta_data)
        ...
        # Calcular puntaje sobre 10
        puntaje_sobre_10 = 0
        if evaluacion.puntaje_total > 0:
            puntaje_sobre_10 = (estudiante_evaluacion.puntaje or 0) / evaluacion.puntaje_total * 10
            puntaje_sobre_10 = round(puntaje_sobre_10, 1)
        
        # Guardar en historial con información más detallada
        historial = HistorialEvaluacion.objects.create(
            ...
            evaluacion_puntaje_total=evaluacion.puntaje_total,
            ...
            detalles={
                ...
                'fecha_inicio_evaluacion': evaluacion.fecha_inicio.isoformat(),
                'duracion_minutos': evaluacion.duracion_minutos,
                'permitir_revision': evaluacion.permitir_revision,
                'mostrar_resultado': evaluacion.mostrar_resultado,
                'ajustes_puntaje': estudiante_evaluacion.ajustes_puntaje or 0,
                
                # Añadir información adicional de ejercicios para mejor visualización
                'ejercicios_ids': [respuesta['ejercicio_id'] for respuesta in respuestas_data],
                'tiene_ejercicios': len(respuestas_data) > 0,
                
                # Propiedades UI - MOVER LA PROPIEDAD COLOR_CLASE A DETALLES
                'color_clase': get_color_clase(puntaje_sobre_10),
                'formattedScore': f"{puntaje_sobre_10:.1f}/10",
                'formattedScoreDisplay': "¡Excelente!" if puntaje_sobre_10 >= 9 else 
        ...
        print(f"DEBUG: Guardados {len(respuestas_data)} ejercicios en el historial")
            
    except Exception as e:
        logger.error(f"Error al guardar evaluación en historial: {str(e)}")
        import traceback
